...
export class SmsConfigService {
  ...
  getAllSms(status:string,limit:number,offset:number): Observable<any> {
  ...
  getAllActiveSms(status:string): Observable<any> {
    const headers = new HttpHeaders().set("Authorization", sessionStorage.getItem("token")!);
