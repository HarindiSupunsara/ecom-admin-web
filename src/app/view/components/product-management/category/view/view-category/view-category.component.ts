...
export class ViewCategoryComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['name','description','displayOrder','createBy','createDate','status','edit'];
  ...
  constructor(private categoryService:CategoryService,private dialog: MatDialog,private routes:ActivatedRoute,private router:Router) { 
  ...
  getAllCategory() {
    ...
    this.categoryService.getCount("").subscribe((response: { data: { Count: number; }; }) => {
      console.log(response);
      this.length = response.data.Count;
      this.categoryService.getAllCategory("ALL",this.paginator.pageSize, this.paginator.pageIndex , 1, 1).subscribe((resp: any[] | null) => {
        this.loading = false;
        console.log(resp);
        if (resp != null) {
          ...
          this.expandedElement = this.categoryList;
  ...
  changeCategoryStatus(row: any, e: any) {
    console.log(e);
    if (!this.approve) {
    ...
    if (this.approve) {
      console.log(row);
      this.approve = false;
      this.categoryService.changeStatus(row.id, row.status).subscribe((response:any) => {
  ...
  alertconfirmation(title: string, message: string[], type: string, method: string, row: any, e: any) {
    ...
    dialogRef.afterClosed().subscribe((result:any) => {
      ...
      if (result.result === "yes") {
        ...
        this.approve = true;
        this.changeCategoryStatus(row, e);
      } else {
        if (row.status == 1) {
          row.status = 0;
        } else {
