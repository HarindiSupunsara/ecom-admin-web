...
export class ViewMenuComponent implements AfterViewInit {
  ...
  allMenuList: Array<any> = new Array();
  ...
  constructor(private permissionService: PermissionService, public dialog: MatDialog, private menuService:MenuService) {
  }
  ...
  getAllMenus() {
    this.loading = true;
    this.allMenuList = new Array();
    this.menuService.getCount("").subscribe((response: { data: { Count: number; }; }) => {
      console.log(response);
      this.length = response.data.Count;
      this.menuService.getAllMenus("ALL",this.paginator.pageSize, this.paginator.pageIndex).subscribe((resp: any[] | null) => {
        this.loading = false;
        console.log(resp);
        if (resp != null) {
          resp.forEach((element: any) => {
            this.allMenuList.push(element);
          });

          this.dataSource = new MatTableDataSource(this.allMenuList);
  ...
  changeMenuStatus(row: any, e: any) {
    console.log(e);
    if (!this.approve) {
    ...
    if (this.approve) {
      console.log(row);
      this.approve = false;
      this.menuService.changeStatus(row.id, row.status).subscribe((response:any) => {
